...
public class DeviceActionRegisterTest {
  @Test
  public void testShouldSucceed() throws JsonMappingException, JsonParseException, IOException {
    ...
    when(request.getParameter(HttpConstants.PARAM_DEVICEID)).thenReturn("abcdef");
    when(request.getParameter(HttpConstants.PARAM_DEVICETYPE)).thenReturn("phone");
    ...
    when(data.registerDevice(1, "abcdef", "phone")).thenReturn(100);

    DeviceActionRegister action = Mockito.spy(new DeviceActionRegister());
    ...
    verify(request, atLeast(1)).getParameter(HttpConstants.PARAM_DEVICEID);
    verify(request, atLeast(1)).getParameter(HttpConstants.PARAM_DEVICETYPE);
  }
}

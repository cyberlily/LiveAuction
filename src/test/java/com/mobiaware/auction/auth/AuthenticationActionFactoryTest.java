...
public class AuthenticationActionFactoryTest {
  ...
  public void testShouldSucceed() {
    ...
    when(request.getParameter(HttpConstants.PARAM_ACTION)).thenReturn(HttpConstants.ACTION_REGISTERDEVICE);
    Assert.assertFalse(AuthenticationActionFactory.getAction(request) instanceof InvalidAction);

    when(request.getParameter(HttpConstants.PARAM_ACTION)).thenReturn(HttpConstants.ACTION_DEVICELIST);
    Assert.assertFalse(AuthenticationActionFactory.getAction(request) instanceof InvalidAction);

    when(request.getParameter(HttpConstants.PARAM_ACTION)).thenReturn(HttpConstants.ACTION_DEVICELISTCNT);
    Assert.assertFalse(AuthenticationActionFactory.getAction(request) instanceof InvalidAction);
  }

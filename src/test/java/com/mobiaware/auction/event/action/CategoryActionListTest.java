...
import com.mobiaware.auction.CategorySerializationTest;
...
public class CategoryActionListTest {
  @Test
  public void testShouldSucceed() throws JsonMappingException, JsonParseException, IOException {
    ...
    when(data.getAuction(1)).thenReturn(AuctionSerializationTest.AUCTION);
    when(data.getCategoryCount(1)).thenReturn(1);
    when(data.getCategories(1, 0, 5, null, null)).thenReturn(ImmutableList.of(CategorySerializationTest.CATEGORY));

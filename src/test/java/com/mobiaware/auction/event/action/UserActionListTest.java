...
public class UserActionListTest {
  @Test
  public void testShouldSucceed() throws JsonMappingException, JsonParseException, IOException {
    ...
    when(data.getUserCount(1)).thenReturn(1);
    when(data.getUsers(1, 0, 5, null, null)).thenReturn(ImmutableList.of(UserSerializationTest.USER));

    UserActionList action = Mockito.spy(new UserActionList());

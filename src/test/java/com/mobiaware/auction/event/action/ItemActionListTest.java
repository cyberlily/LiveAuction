...
import com.mobiaware.auction.ItemSerializationTest;
import com.mobiaware.auction.data.DataService;
import com.mobiaware.servlet.HttpConstants;

public class ItemActionListTest {
  @Test
  public void testShouldSucceed() throws JsonMappingException, JsonParseException, IOException {
    HttpServletRequest request = Mockito.mock(HttpServletRequest.class);
    when(request.getParameter(HttpConstants.PARAM_AUCTIONUID)).thenReturn("1");
    when(request.getParameter(HttpConstants.PARAM_CATEGORYUID)).thenReturn("1");
    ...
    when(data.getItemCount(1, 1)).thenReturn(1);
    when(data.getItems(1, 1, 0, 5, null, null)).thenReturn(ImmutableList.of(ItemSerializationTest.ITEM));

    ItemActionList action = Mockito.spy(new ItemActionList());
    Mockito.when(action.getDataService()).thenReturn(data);

    action.perform(request, response);

    verify(request, atLeast(1)).getParameter(HttpConstants.PARAM_AUCTIONUID);
    verify(request, atLeast(1)).getParameter(HttpConstants.PARAM_CATEGORYUID);

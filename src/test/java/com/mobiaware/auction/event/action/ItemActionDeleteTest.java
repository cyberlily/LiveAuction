...
public class ItemActionDeleteTest {
  @Test
  public void testShouldSucceed() throws IOException {
    HttpServletRequest request = Mockito.mock(HttpServletRequest.class);
    when(request.getParameter(HttpConstants.PARAM_ITEMUID)).thenReturn("1");
    ...
    Mockito.when(data.deleteItem(1)).thenReturn(1);

    ItemActionDelete action = Mockito.spy(new ItemActionDelete());
    Mockito.when(action.getDataService()).thenReturn(data);

    action.perform(request, response);

    verify(request, atLeast(1)).getParameter(HttpConstants.PARAM_ITEMUID);
  }
}

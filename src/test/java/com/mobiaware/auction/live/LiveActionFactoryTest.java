...
public class LiveActionFactoryTest {
  ...
  public void testShouldSucceed() {
    ...
    when(request.getParameter(HttpConstants.PARAM_ACTION)).thenReturn(HttpConstants.ACTION_MYWATCHITEMLIST);
    Assert.assertFalse(LiveActionFactory.getAction(request) instanceof InvalidAction);

    when(request.getParameter(HttpConstants.PARAM_ACTION)).thenReturn(HttpConstants.ACTION_MYBIDITEMLIST);
    Assert.assertFalse(LiveActionFactory.getAction(request) instanceof InvalidAction);

    when(request.getParameter(HttpConstants.PARAM_ACTION)).thenReturn(HttpConstants.ACTION_ITEMUPDATES);
    Assert.assertFalse(LiveActionFactory.getAction(request) instanceof InvalidAction);
    ...
    when(request.getParameter(HttpConstants.PARAM_ACTION)).thenReturn(HttpConstants.ACTION_FUNDACAUSE);
    Assert.assertFalse(LiveActionFactory.getAction(request) instanceof InvalidAction);

    when(request.getParameter(HttpConstants.PARAM_ACTION)).thenReturn(HttpConstants.ACTION_DEVICEMESSAGE);

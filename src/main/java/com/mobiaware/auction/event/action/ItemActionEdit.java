...
public class ItemActionEdit extends Action {
  ...
  public void perform(final HttpServletRequest request, final HttpServletResponse response) {
    ...
    Item item =
        Item.newBuilder().auctionUid(auction.getUid()).itemNumber(pitemnumber).name(pname).category(pcategory)
            .description(pdescription).seller(pseller).valPrice(pvalprice).minPrice(pminprice).incPrice(pincprice)
            .build();

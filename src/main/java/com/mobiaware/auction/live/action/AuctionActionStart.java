...
public class AuctionActionStart extends Action {
  ...
  public void perform(final HttpServletRequest request, final HttpServletResponse response) {
    ...
    DateTime tomorrow = today.plusDays(30); // give it a month

    Auction auction = getDataService().getAuction(pauctionuid);
    auction = auction.toBuilder().startDate(today.getMillis()).endDate(tomorrow.getMillis()).build();
